  ...
  stages {
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep notif-middleware | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep notif-middleware | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/notif-middleware:latest/notif-middleware:$feature_name/g" cmd/notif-middleware/k8s/02-notif-middleware.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/notif-middleware/k8s/02-notif-middleware.yaml'
