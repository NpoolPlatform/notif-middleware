...
import (
	...
	entamt "github.com/NpoolPlatform/notif-manager/pkg/db/ent/announcement"
...
func join(stm *ent.SendAnnouncementQuery) *ent.SendAnnouncementSelect {
	return stm.Select().Modify(func(s *sql.Selector) {
		...
		s.
			...
			AppendSelect(
				sql.As(t1.C(entamt.FieldID), "announcement_id"),
				t1.C(entamt.FieldTitle),
				t1.C(entamt.FieldContent),
				t1.C(entamt.FieldType),
			)
	})
}
